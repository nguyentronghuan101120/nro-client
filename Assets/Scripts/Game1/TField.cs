namespace Game1
{
    ...
    public class TField : IActionListener
    {
        ...
        public long lastTimeCheckVisible;
        ...
        public void paintInputTf(mGraphics g, bool iss, int x, int y, int w, int h, int xText, int yText, string text, string info)
        {
            ...
            if (Main.isPC && selectStartIndex > -1)
            {
                g.setColor(new Color(0, 0, 0, .4f));
                string selectedText = text.Substring(Mathf.Min(selectStartIndex, caretPos), Mathf.Abs(selectStartIndex - caretPos));
                if (inputType == INPUT_TYPE_PASSWORD)
                    selectedText = new string('*', selectedText.Length);
                g.fillRect(xText + mFont.tahoma_8b.getWidth(paintedText.Substring(0, Mathf.Min(selectStartIndex, caretPos))) - 1, yText, mFont.tahoma_8b.getWidth(selectedText), mFont.tahoma_8b.getHeight());
            }
                ...
                switch (Event.current.keyCode)
                {
                    ...
                    case KeyCode.Backspace:
                        ...
                        else
                        {
                            if (text.Length > 0)
                            {
                                int newCaretPos = Mathf.Min(caretPos, selectStartIndex);
                                text = text.Substring(0, Mathf.Min(caretPos, selectStartIndex)) + text.Substring(Mathf.Max(caretPos, selectStartIndex));
                    ...
                    default:
                        if (Event.current.control)
                        {
                            ...
                            else if ((Event.current.keyCode == KeyCode.C || Event.current.keyCode == KeyCode.Insert) && inputType != INPUT_TYPE_PASSWORD)
                            {
                                if (selectStartIndex < 0)
                                    ...
                                    GUIUtility.systemCopyBuffer = text.Substring(Mathf.Min(caretPos, selectStartIndex), Mathf.Abs(caretPos - selectStartIndex));
                                break;
                            }
                            else if (Event.current.keyCode == KeyCode.V)
                            {
                                ...
                                {
                                    int newCaretPos = Mathf.Min(caretPos, selectStartIndex);
                                    text = text.Substring(0, Mathf.Min(caretPos, selectStartIndex)) + GUIUtility.systemCopyBuffer + text.Substring(Mathf.Max(caretPos, selectStartIndex));
                        ...
                        {
                            int newCaretPos = Mathf.Min(caretPos, selectStartIndex);
                            text = text.Substring(0, Mathf.Min(caretPos, selectStartIndex)) + c + text.Substring(Mathf.Max(caretPos, selectStartIndex));
                            caretPos = ++newCaretPos;
                        }
                        selectStartIndex = -1;
                        changed = true;
